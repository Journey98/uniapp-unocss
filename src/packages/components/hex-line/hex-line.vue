<template>
  <view class="hex-line" :style="[lineStyle]"> </view>
</template>

<script>
/**
 * line 线条
 * @description 此组件一般用于显示一根线条，用于分隔内容块
 ...
 * @example <hex-line color="red"></hex-line>
 */
export default {
  name: "hex-line",
  props: {
  ...
  computed: {
    lineStyle() {
      ...
      if (this.direction == "row") {
        ...
        style.width = this.$hex.addUnit(this.length);
        if (this.hairLine) style.transform = "scaleY(0.5)";
      } else {
        // 如果是竖向线条，边框宽度为1px，再通过transform缩小一半，就是0.5px了
        style.borderLeftWidth = "1px";
        style.borderLeftStyle = this.borderStyle;
        style.height = this.$hex.addUnit(this.length);
