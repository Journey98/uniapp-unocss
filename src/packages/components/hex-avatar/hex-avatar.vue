<template>
  <view class="hex-avatar" :style="[wrapStyle]" @tap="click">
    <image
      @error="loadError"
      :style="[imgStyle]"
      class="hex-avatar__img"
      v-if="!hexText && avatar"
      :src="avatar"
      :mode="imgMode"
    ></image>
    <text
      ...
      v-else-if="hexText"
      :style="{
        fontSize: fontSize ? fontSize + 'rpx' : fontSizeObj[size],
      }"
    >
      {{ hexText }}
    </text>
 ...
 * @example <hex-avatar :src="src"></hex-avatar>
 */
export default {
  name: "hex-avatar",
  emits: ["click"],
  ...
  computed: {
    ...
    hexText() {
      if (this.isEndStr) {
        if (this.text.length < this.subLength) {
          return this.text;
        } else {
          let str = this.text.substring(this.text.length - this.subLength);
          return str;
        }
      } else {
        let str = this.text.slice(0, this.subLength);
        return str;
      }
    },
  },
